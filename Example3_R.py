#----------------------------------------------------------------------#
#                     EXAMPLE 3 (RECTANGULAR DOMAIN)                   #
#                                                                      #
#   This program solve the Poisson equation:                           #
#             (beta*u_x)_x + (beta*u_y)_y + kappa*u= 0,                #
#   where                                                              #
#              beta=sin(x+y) and kappa(x,y)=-(x**2+y**2)               #
...
class PhysicsInformedNN:
    ...
    def net_f(self,x,y):
        ...
        betax = tf.sin(tf.add(x,y))
        betay = tf.sin(tf.add(x,y))
        kappa = -(x*x+y*y)
        ubx   = betax*u_x
        uby   = betay*u_y
        u_xx  = tf.gradients(ubx,x)[0]
        u_yy  = tf.gradients(uby,y)[0]
        f = u_xx + u_yy + kappa*u
        return f
    #-----------------------------------------------
...
if __name__ == "__main__": 
    ...
    #-----------------------------------------------
    N_u = 40    # No. of training data     
    N_f = 6400  # No. of collocation points
    #-----------------------------------------------
    NN = 80    
    ...
    #-----------------------------------------------
    #  Examples
    TestCase = 2  # 1)exp*sin, 2)sin*sin
    ...
    for j in range(0,Ny):
        for i in range(0,Nx):
            Exact[j,i] = np.sin(np.pi*x[i])*np.sin(np.pi*y[j])
    #---------------
    # Exact(Npx,Npy)
    Exactp = np.zeros((Npy,Npx),float)
    for j in range(0,Npy):
        for i in range(0,Npx):
            Exactp[j,i] = np.sin(np.pi*xp[i])*np.sin(np.pi*yp[j]) 
    ...
    #-----------------------------------------------
    # [2] Collocation points: (X_f_train,rhs_train)
    #----------------
    ...
    betax = np.zeros((N_f,1),float)
    betay = np.zeros((N_f,1),float)
    kappa = np.zeros((N_f,1),float)
    for l in range(0,N_f):
        kappa[l] = -(xf[l]**2+yf[l]**2)
        #----  
        betax[l] = np.sin(xf[l]+yf[l])
        betay[l] = np.sin(xf[l]+yf[l])
        betax_x  = np.cos(xf[l]+yf[l])
        betay_y  = np.cos(xf[l]+yf[l])
        #----    
        uu   = np.sin(np.pi*xf[l])*np.sin(np.pi*yf[l])
        u_x  = np.pi*np.cos(np.pi*xf[l])*np.sin(np.pi*yf[l])
        u_y  = np.pi*np.sin(np.pi*xf[l])*np.cos(np.pi*yf[l])
        u_xx = -(np.pi**2)*uu
        u_yy = -(np.pi**2)*uu
        #----
        buxx = u_x*betax_x+betax[l]*u_xx
        buyy = u_y*betay_y+betay[l]*u_yy
        rhs[l] = buxx + buyy + kappa[l]*uu
    rhs_train = rhs

    #-----------------------------------------------
    # [1] Training boundary data: Dirichlet (xu,yu,u)
    ...
    print('---------------------------------------------')
    print('  TRAINING:                                  ')
    print('  No. of training points           N_u   =',N_u)
    ...
    # -------------------------------
    # Final results
    file1 = open("output/dataR/Results_Ex3.txt","w")
    file1.write('-------------------------------------------------\n')
    file1.write('Example 3: Variable \n')
    file1.write('-------------------------------------------------\n')
    file1.write('Nu = %s\n' % N_u)
    file1.write('Nf = %s '  % N_f )
    ...
    for l in range(0,N_f):
        ...
        file3.write('%f  ' % yf[l])
        file3.write('%f  ' % rhs[l])
        file3.write('%f  ' % betax[l])
        file3.write('%f  ' % betay[l])
        file3.write('%f\n' % kappa[l])
    file3.close()
